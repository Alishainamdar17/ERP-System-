...
import { createTransaction } from "../Services/cashTrackerService";
...
const s = {
  ...
  modal: { background: "#fff", borderRadius: 10, width: 460, border: "0.5px solid #e5e5e0", boxShadow: "0 8px 32px rgba(0,0,0,0.12)" },
...
function AddTransactionModal({ projects = [], onClose, onCreated }) {
  const today = new Date().toISOString().split("T")[0];
  const [form, setForm] = useState({ projectId: "", type: "INCOME", amount: "", transactionDate: today, description: "" });
        ...
        <div style={s.body}>
          <div style={s.row}>
            <div style={s.group}>
              ...
              <select style={s.input} name="projectId" value={form.projectId} onChange={handleChange}>
          ...
          <div style={s.row}>
            ...
            <div style={s.group}>
              ...
              <input style={s.input} name="transactionDate" type="date" value={form.transactionDate} onChange={handleChange} />
            </div>
          </div>
          <div style={s.group}>
            <label style={s.label}>Description</label>
            <input style={s.input} name="description" placeholder="What is this for?" value={form.description} onChange={handleChange} />
          </div>
